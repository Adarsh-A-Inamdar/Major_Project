...
\usepackage{url}
\usepackage{color}
% \usepackage{listings} % Removed code listings package
...
\author{
    Anmol Kundap, Adarsh A Inamdar, Tanushree M Pujar, Amogh K Baliga \\
    \textit{Department of Computer Science and Engineering} \\
    \textit{Bapuji Institute of Engineering and Technology, Davangere, Karnataka, India} \\
    Emails: anmolkundap2004@gmail.com, eradarshainamdar@gmail.com,\\
    tanushreempujar@gmail.com, amoghbaliga254257@gmail.com \\
    \\
    \textbf{Supervisor:} Dr. Pradeep N \\
...
The remainder of this paper is organized as follows: Section 2 provides a comprehensive literature review. Section 3 outlines the methodology and proposed system architecture. Section 4 details the background and domain overview. Section 5 presents the experimental results and analysis. Finally, Section 6 concludes the study.
...
\begin{enumerate}
    \item \textbf{Diagnostic Fragmentation:} Current systems often perform only binary classification or distinguish between a limited set of subtypes (e.g., only ALL vs. Normal). There is a lack of unified frameworks that can handle the full spectrum of leukemia types (ALL, AML, CLL, CML) and provide granular severity grading in a single inference pass.
    
    \item \textbf{Hyperparameter Sensitivity:} Deep learning models are notoriously sensitive to hyperparameters. An inappropriate learning rate can cause the model to diverge or get stuck in local minima. A suboptimal batch size can lead to poor generalization. Manual tuning (trial and error) is exhaustive and rarely finds the global optimum.
    
    \item \textbf{Data Heterogeneity:} Medical images are highly variable. A model must be robust enough to handle differences in staining and resolution. Standard CNNs often overfit to the source domain.
    
    \item \textbf{Interpretability:} The "black box" nature of deep learning is a barrier to adoption. Clinicians need more than just a label; they need confidence scores and an understanding of severity.
\end{enumerate}
...
\textit{Resizing}: The input images $I$ of varying dimensions $H \times W \times C$ are resized to a fixed dimension of $224 \times 224 \times 3$ to match the input requirements of ResNet-18. We use Bilinear Interpolation for this operation, defined for a point $(x, y)$ as:
\begin{equation}
...
\textit{Normalization}: We normalize the pixel values to have a mean of 0 and a standard deviation of 1. Let $I_{i,j,c}$ be the pixel value at row $i$, column $j$, and channel $c$. The normalized value $\hat{I}_{i,j,c}$ is calculated as:
\begin{equation}
...
\begin{enumerate}
    \item \textbf{Image Acquisition:} Capturing high-resolution images of blood smears.
    \item \textbf{Preprocessing:} Enhancing image quality through noise reduction and color normalization.
    \item \textbf{Segmentation:} Isolating individual white blood cells from the background (erythrocytes and plasma).
    \item \textbf{Feature Extraction:} Deriving quantitative descriptors of the cells.
    \item \textbf{Classification:} Assigning a diagnostic label to the cell or the patient.
\end{enumerate}
...
\begin{enumerate}
    \item \textbf{ALL-IDB (Acute Lymphoblastic Leukemia Image Database):} Provided by the University of Milan. It contains 260 segmented images of lymphoblasts.
    \item \textbf{C-NMC (Classification of Normal vs Malignant Cells):} A large dataset from the ISBI 2019 challenge, providing high-quality single-cell images.
    \item \textbf{BCCD (Blood Cell Count and Detection):} Used primarily for the 'normal' and 'abnormal' distinctions to broaden the class distribution.
\end{enumerate}
...
To provide a deeper insight into the model's performance, we generated several visualizations.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{outputs/figures/Fig_6.1_ROC_Comparison.png}
    \caption{ROC-AUC Comparison of Activation Functions. The proposed ReLU+PSO approach (Green) demonstrates superior area under the curve compared to standard activations.}
    \label{fig:roc_curve}
\end{figure}

Figure \ref{fig:roc_curve} illustrates the ROC curves for different activation functions. Our proposed method outperforms standard ReLU and Sigmoid functions, substantiating the effectiveness of the PSO optimization.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{outputs/figures/Fig_6.2_Activation_Accuracy.png}
    \caption{Activation-wise Test Accuracy. The proposed method achieves the highest accuracy of 94.2\%.}
    \label{fig:activation_acc}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{outputs/figures/Fig_6.6_Confusion_Matrix.png}
    \caption{Confusion Matrix for Leukemia Subtypes. Shows high diagonal values, indicating strong classification performance across all four classes.}
    \label{fig:confusion_matrix}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{outputs/figures/Fig_6.7_Loss_Trend.png}
    \caption{Loss Trend Across Epochs. The training and validation loss decrease consistently, indicating stable learning without significant overfitting.}
